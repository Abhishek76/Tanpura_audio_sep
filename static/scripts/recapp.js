window.onload = function() {
    ...
    function AudioAndUpload(blob) {
        ...
        predictResult.innerHTML = "start"+ '   Predicted:' + '...'+'<br>'; //initialize the prediction results
        ...
        function predictFunc(){
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    predictResult.innerHTML ='scale predicted '+ data.message+' scale  file generated ' + data.predicted+'<br>';
                    var soun      = document.createElement('audio');
                    soun.id       = 'audio-player';
                    soun.controls = 'controls';
                    soun.src      = "http://0.0.0.0:8000/"+ data.predicted
                    soun.load(); //call this to just preload the audio without playing
                   // soun.play()
                    //"http://127.0.0.1:8000/vocal/scpyvoc.wav"
                    //"http://127.0.0.1:8000/tedst/myupload.wav"
                    //"http://0.0.0.0:8000/"+"vocal/2019-10-27T141436.662Z_vocal.wav";
                    
                    soun.type     = 'audio/mpeg';
                    recordingsList.appendChild(soun);
                },
                error: function(e) {
